...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($temp_name, $folder)) {
            $updateFields[] = "image_path = '$image_name'";
...
<head>
    ...
    <style>
        ...
        .form-container { background: white; padding: 30px; border-radius: 8px; width: 400px; margin: 50px auto; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
        ...
        button { background: #2980b9; color: white; border: none; padding: 12px; width: 100%; border-radius: 4px; cursor: pointer; font-size: 16px; }
        .current-image { display: block; margin-bottom: 15px; width: 100%; max-height: 230px; object-fit: cover; border-radius: 4px; }
    </style>
</head>
<body>
...
<div class="form-container">
    ...
    <form action="edit_item.php?id=<?php echo $item['id']; ?>" method="POST" enctype="multipart/form-data">
        ...
        <select name="category" required>
            <?php if (mysqli_num_rows($categories) > 0): ?>
                <?php while ($cat = mysqli_fetch_assoc($categories)): ?>
                    <option value="<?php echo htmlspecialchars($cat['name']); ?>" <?php echo $item['category'] === $cat['name'] ? 'selected' : ''; ?>>
        ...
        <?php if (!empty($item['image_path'])): ?>
            <img class="current-image" src="uploads/<?php echo htmlspecialchars($item['image_path']); ?>" alt="Current Image">
        <?php endif; ?>

        <label style="font-size: 14px; color: #666;">Upload New Image (optional):</label>
